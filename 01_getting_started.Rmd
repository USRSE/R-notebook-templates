...
You will learn how to use Quarto in order to submit a reproducible document to the US-RSE conference. The aim is to compile `reproducible_document_template.qmd` into an HTML document, which is the accepted format for reproducible documents within the digital proceedings.
...
```{bash}
quarto render reproducible_document_template.qmd --to html
```
...
```{bash}
quarto preview reproducible_document_template.qmd
```

### 4. Observe reflected changes to reproducible_document_template.qmd

We want to see a change reflected when we alter the notebook.

- [Open reproducible_document_template.qmd](reproducible_document_template.qmd) for editing
